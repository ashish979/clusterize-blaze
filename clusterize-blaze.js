;(function(name, definition) {
  ...
  Clusterize.prototype = {
    ...
    exploreEnvironment: function(template, rows, cache, otherArgs) {
      var opts = this.options;
      opts.content_tag = this.content_elem.tagName.toLowerCase();
      if( ! rows.length) return;
      if(ie && ie <= 9 && ! opts.tag) opts.tag = rows[0].match(/<([^>\s/]*)/)[1].toLowerCase();

      var div = this.renderBlazeElement(template, rows[0], otherArgs)
      if(this.content_elem.children.length <= 1) {
        this.content_elem.appendChild(div);
        cache.data = div
      }
    ...
    insertToDOM: function(template, rows, cache, otherArgs) {
      ...
      var data = this.generate(rows, this.getClusterNum(), template, otherArgs),
